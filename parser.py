# pip install feedparser (must be > v5.0.1)
# for arxiv.org
...
class Parser:
    ...
    def __call__(self, search_query, max_results=None):
        ...
        query = 'search_query=%s&start=%i&max_results=%i' % (search,
                                                             self.start,
                                                             max_results)
        ...
        for entry in feed.entries:
            ...
            for link in entry.links:
                if link.rel == 'alternate':
                    ...
                    pdf_link = link.href
            article = self.create_article(id=entry.id.split('/abs/')[-1],
    ...
    def create_article(self, id, date, title, authors, 
                             abs_link, pdf_link, summary):
        """
